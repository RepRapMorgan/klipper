...
class MorganScaraKinematics:
    ...
    def get_calibration(self):
        ...
        stepdists = [rail.get_steppers()[0].get_step_dist()
                     for rail in self.rails]
        return MorganScaraCalibration(self.radius, self.angles, self.arm_lengths,
                                endstops, stepdists)
...
class MorganScaraCalibration:
    ...
    def new_calibration(self, params):
        ...
        angles = [params['angle_'+a] for a in 'abz']
        arms = [params['arm_'+a] for a in 'abz']
        endstops = [params['endstop_'+a] for a in 'abz']
        stepdists = [params['stepdist_'+a] for a in 'abz']
        return MorganScaraCalibration(radius, angles, arms, endstops, stepdists)
