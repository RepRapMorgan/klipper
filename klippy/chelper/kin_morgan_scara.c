...
morgan_scara_stepper_a_calc_position(struct stepper_kinematics *sk, struct move *m, double move_time)
{
    ...
    D = fmin(fmax(D, -ms->D_limit), ms->D_limit); // Clamp D to prevent sqrt from returning NaN and collisions
    ...
    // Calculate theta
    double theta = atan2(y, x) - atan2(ms->L2 * sin(psi), ms->L1 + ms->L2 * cos(psi));
